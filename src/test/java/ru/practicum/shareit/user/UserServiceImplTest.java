...
class UserServiceImplTest {
    ...
    void createUser_WhenUserDtoIsIncorrect_ThenThrowValidationException() {
        ...
        UserDto inputDto5 = new UserDto(0, null, null);
        UserDto inputDto6 = new UserDto(0, "  ", "");
        ...
        assertThatExceptionOfType(ValidationException.class)
                .as("Проверка добавления пользователя с null почтой и null именем")
                .isThrownBy(() -> userService.createUser(inputDto5))
                .withMessage(Constants.NOT_EMPTY_USER_NAME_MESSAGE);

        assertThatExceptionOfType(ValidationException.class)
                .as("Проверка добавления пользователя с пустыми почтой и именем")
                .isThrownBy(() -> userService.createUser(inputDto6))
                .withMessage(Constants.NOT_EMPTY_USER_NAME_MESSAGE);

        Mockito.verifyNoInteractions(userStorage);
    }
