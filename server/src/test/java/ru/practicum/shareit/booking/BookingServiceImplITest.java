...
class BookingServiceImplITest {
    ...
    void getBookingsByBookerId_WhenALLSearchingState_ThenReturnAllBookings() {
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByBookerId(user3.getId(), SearchingState.ALL,
                    0, 20);
            assertThat(requests)
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByBookerId(user2.getId(), SearchingState.ALL,
                    0, 20);
            assertThat(requests)
    ...
    void getBookingsByBookerId_WhenPASTSearchingState_ThenReturnPastBookings() {
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByBookerId(user1.getId(), SearchingState.PAST,
                    0, 20);
            assertThat(requests)
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByBookerId(user2.getId(), SearchingState.PAST,
                    0, 20);
            assertThat(requests)
                    .as("Проверка получения не пустого списка бронирований, когда у пользователя есть" +
    ...
    void getBookingsByBookerId_WhenFUTURESearchingState_ThenReturnFutureBookings() {
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByBookerId(user1.getId(), SearchingState.FUTURE,
                    0, 20);
            assertThat(requests)
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByBookerId(user2.getId(), SearchingState.FUTURE,
                    0, 20);
            assertThat(requests)
                    .as("Проверка получения не пустого списка бронирований, когда у пользователя есть" +
    ...
    void getBookingsByBookerId_WhenCURRENTSearchingState_ThenReturnCurrentBookings() {
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByBookerId(user1.getId(), SearchingState.CURRENT,
                    0, 20);
            assertThat(requests)
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByBookerId(user2.getId(), SearchingState.CURRENT,
                    0, 20);
            assertThat(requests)
                    .as("Проверка получения не пустого списка бронирований, когда у пользователя есть" +
    ...
    void getBookingsByBookerId_WhenWAITINGSearchingState_ThenReturnWaitingBookings() {
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByBookerId(user1.getId(), SearchingState.WAITING,
                    0, 20);
            assertThat(requests)
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByBookerId(user2.getId(), SearchingState.WAITING,
                    0, 20);
            assertThat(requests)
                    .as("Проверка получения не пустого списка бронирований, когда у пользователя есть" +
    ...
    void getBookingsByBookerId_WhenREJECTEDSearchingState_ThenReturnRejectedBookings() {
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByBookerId(user1.getId(), SearchingState.REJECTED,
                    0, 20);
            assertThat(requests)
                    .as("Проверка получения пустого списка бронирований, когда у пользователя нет" +
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByBookerId(user2.getId(), SearchingState.REJECTED,
                    0, 20);
            assertThat(requests)
                    .as("Проверка получения не пустого списка бронирований, когда у пользователя есть" +
    ...
    void getBookingsByOwnerId_WhenALLSearchingState_ThenReturnAllBookings() {
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByOwnerId(user3.getId(), SearchingState.ALL,
                    0, 20);
            assertThat(requests)
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByOwnerId(user1.getId(), SearchingState.ALL,
                    0, 20);
            assertThat(requests)
                    .as("Проверка получения не пустого списка бронирований, когда у пользователя есть" +
                            " бронирования вещей")
    ...
    void getBookingsByOwnerId_WhenPASTSearchingState_ThenReturnPastBookings() {
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByOwnerId(user2.getId(), SearchingState.PAST,
                    0, 20);
            assertThat(requests)
                    .as("Проверка получения пустого списка бронирований, когда у пользователя нет прошедших" +
                            " бронирований вещей")
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByOwnerId(user1.getId(), SearchingState.PAST,
                    0, 20);
            assertThat(requests)
                    .as("Проверка получения не пустого списка бронирований, когда у пользователя есть" +
    ...
    void getBookingsByOwnerId_WhenFUTURESearchingState_ThenReturnFutureBookings() {
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByOwnerId(user2.getId(), SearchingState.FUTURE,
                    0, 20);
            assertThat(requests)
                    .as("Проверка получения пустого списка бронирований, когда у пользователя нет будущих" +
                            " бронирований вещей")
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByOwnerId(user1.getId(), SearchingState.FUTURE,
                    0, 20);
            assertThat(requests)
                    .as("Проверка получения не пустого списка бронирований, когда у пользователя есть" +
    ...
    void getBookingsByOwnerId_WhenCURRENTSearchingState_ThenReturnCurrentBookings() {
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByOwnerId(user2.getId(), SearchingState.CURRENT,
                    0, 20);
            assertThat(requests)
                    .as("Проверка получения пустого списка бронирований, когда у пользователя нет текущих" +
                            " бронирований вещей")
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByOwnerId(user1.getId(), SearchingState.CURRENT,
                    0, 20);
            assertThat(requests)
                    .as("Проверка получения не пустого списка бронирований, когда у пользователя есть" +
    ...
    void getBookingsByOwnerId_WhenWAITINGSearchingState_ThenReturnWaitingBookings() {
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByOwnerId(user2.getId(), SearchingState.WAITING,
                    0, 20);
            assertThat(requests)
                    .as("Проверка получения пустого списка бронирований, когда у пользователя нет" +
                            " бронирований, ожидающих подтверждение")
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByOwnerId(user1.getId(), SearchingState.WAITING,
                    0, 20);
            assertThat(requests)
                    .as("Проверка получения не пустого списка бронирований, когда у пользователя есть" +
                            " бронирования, ожидающие подтверждение")
    ...
    void getBookingsByOwnerId_WhenREJECTEDSearchingState_ThenReturnRejectedBookings() {
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByOwnerId(user2.getId(), SearchingState.REJECTED,
                    0, 20);
            assertThat(requests)
                    .as("Проверка получения пустого списка бронирований, когда у пользователя нет" +
                            " отклоненных бронирований")
        ...
        assertThatCode(() -> {
            List<BookingDto> requests = bookingService.getBookingsByOwnerId(user1.getId(), SearchingState.REJECTED,
                    0, 20);
            assertThat(requests)
                    .as("Проверка получения не пустого списка бронирований, когда у пользователя есть" +
                            " отклоненные бронирования")
